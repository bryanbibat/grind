class Mission < ActiveRecord::Base
  ...
  def attempt(user)
    ...
    if try_mission(user)
      ...
      @mission_messages = ["Mission successful",
        "You gained #{number_to_currency @credits_reward } credits and #{ @xp_reward } XP"]
    else
  ...
  def user_points(user)
    user_points = 0
    if check_melee?
      user_melee = (user.effective_melee >= max_melee) ? max_melee : user_melee
      user_points += user_melee - min_melee + 1
    end
    if check_ranged?
      user_ranged = (user.effective_ranged >= max_ranged) ? max_ranged : user_ranged
      user_points += user_ranged - min_ranged + 1
    end
    if check_defense?
      user_defense = (user.effective_defense >= max_defense) ? max_defense : user_defense
      user_points += user_defense - min_defense + 1
    end
    if check_agility?
      user_agility = (user.effective_agility >= max_agility) ? max_agility : user_agility
      user_points += user_agility - min_agility + 1
    end
    if check_cunning?
      user_cunning = (user.effective_cunning >= max_cunning) ? max_cunning : user_cunning
